...
class ClientApiHelper(object):
    ...
    def validate_data(self, project, data):
        ...
        # TODO because level is an anyOf number/string, does the defaulting work?
        # TODO need to figure out numeric (or numeric string) levels before we can delete this
        ...
        if isinstance(level, six.string_types) and not level.isdigit():
            ...
            try:
                ...
                self.log.debug('Discarded invalid level value: %s', level)
                errors.append(
                    {
                        'type': EventError.INVALID_DATA,
                        'name': 'level',
